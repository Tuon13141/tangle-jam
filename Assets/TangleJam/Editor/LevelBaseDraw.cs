...
namespace Tuon
{
    ...
    public class LevelBaseDraw : Editor
    {
        ...
        private List<Color> colorsInStage = new List<Color>();
        ...
        void SelectDraw()
        {
            ...
            EditorGUILayout.BeginHorizontal(); // Start horizontal layout
            {
                for (int i = 0; i < levelBase.PixelData.Colors.Count(); i++)
                {
                    ...
                    if (colorsInStage.Contains(color) == false) continue;
            ...
            if (EditorGUI.DropdownButton(rect, new GUIContent(levelBase.Stage.tutorialType.ToString()), FocusType.Keyboard))
            {
                ...
                foreach (LevelAsset.TutorialType type in System.Enum.GetValues(typeof(LevelAsset.TutorialType)))
                {
                    ...
                    {
                        levelBase.Stage.tutorialType = type;
                    });
                }
                menu.DropDown(rect);
            }
            ...
            if (EditorGUI.DropdownButton(rect1, new GUIContent(levelBase.Stage.difficulty.ToString()), FocusType.Keyboard))
            {
                ...
                foreach (LevelAsset.Difficulty type in System.Enum.GetValues(typeof(LevelAsset.Difficulty)))
                {
                    ...
                    {
                        levelBase.Stage.difficulty = type;
                    });
                }
                menu.DropDown(rect1);
            }
        ...
        void SetupValueGrid(int index, StageData.CellData info)
        {
            ...
            if (Event.current.button == 0)
            {
                ...
                {
                    case StageData.CellType.Empty:
                        levelBase.Stage.Data[index].Type = StageData.CellType.Empty;
                        break;
                    case StageData.CellType.Wall:
                        levelBase.Stage.Data[index].Type = StageData.CellType.Wall;
                        break;

                    case StageData.CellType.Coil:
                        levelBase.Stage.Data[index].Type = StageData.CellType.Coil;
                        levelBase.Stage.Data[index].Value = colorSelect;
                        break;

                    case StageData.CellType.Stack:
                        levelBase.Stage.Data[index].Type = StageData.CellType.Stack;
                        levelBase.Stage.Data[index].Direction = directionSelect;
                        break;

                    case StageData.CellType.CoilLocked:
                        levelBase.Stage.Data[index].Type = StageData.CellType.CoilLocked;
                        levelBase.Stage.Data[index].Value = colorSelect;
                        break;

                    case StageData.CellType.PinWall:
                        levelBase.Stage.Data[index].Type = StageData.CellType.PinWall;
                        levelBase.Stage.Data[index].Direction = directionSelect;
                        break;
                    case StageData.CellType.PinControl:
                        levelBase.Stage.Data[index].Type = StageData.CellType.PinControl;
                        levelBase.Stage.Data[index].Direction = directionSelect;
                        break;
                    case StageData.CellType.CoilPair:
                        levelBase.Stage.Data[index].Type = StageData.CellType.CoilPair;
                        levelBase.Stage.Data[index].Value = colorSelect;
                        levelBase.Stage.Data[index].Direction = directionSelect;
                        break;

                    case StageData.CellType.ButtonStack:
                        levelBase.Stage.Data[index].Type = StageData.CellType.ButtonStack;
                        break;
                    case StageData.CellType.Key:
                        levelBase.Stage.Data[index].Type = StageData.CellType.Key;
                        break;
                    case StageData.CellType.Lock:
                        levelBase.Stage.Data[index].Type = StageData.CellType.Lock;
                        break;
                }
            }
            else if (Event.current.button == 1)
            {
                ...
                EditorMenuDraw.ShowWindow(info, data => levelBase.Stage.Data[index] = data);
            }
        }
        ...
        {
            var currentStage = levelBase.Stage;
        ...
        void CheckLevel()
        {
            ...
            for (int y = 0; y < stageMap.Height; y++)
            {
                for (int x = 0; x < stageMap.Width; x++)
                {
                    ...
                    if (dataElement.Type == StageData.CellType.Stack)
                    {
                        ...
                        {
                            Debug.LogError($"Stage 0 -Stack Error!");
                        }
            ...
            Debug.Log($"stage 0 - {string.Join(",", group.Select(x => x.Key.ToString()))}");
            if (group.Count != levelBase.PixelData.Colors.Length)
            {
                ...
                if (missingColors.Count > 0) Debug.LogError($"stage 0 - {string.Join(",", missingColors.Select(x => colorList.IndexOf(x).ToString()))}");
            ...
            levelBase.Stage.RandomCoils = string.Join(",", listCoil.OrderBy(o => o.color).Select(x => x.color).ToArray());
            s = string.Format("{0}\n - stage {1}: {2} : {3} ->\n{4}", s, 1, group.Count(), totalDifficulty, string.Join("\n", group.Select(x => string.Format("      {0}: {1}", x.Key, x.Count()))));
