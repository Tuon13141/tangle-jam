	...
	) : SV_Target
{
	...
	{
		color = dot(color, _HiddenColor) * _HiddenBaseBaseColor;
	}
	else if(_RevealProgress > 0)
	{
		float normalizedY = 1 - (input.worldPos.y - _BoundsMinY) / (_BoundsMaxY - _BoundsMinY);

		if (normalizedY < _RevealProgress)
		{
			if (normalizedY + _BoundaryThickness < _RevealProgress )
			{
				color = dot(color, _HiddenColor) * _HiddenBaseBaseColor;
			}
			else
