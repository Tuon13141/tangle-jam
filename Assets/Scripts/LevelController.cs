...
public class LevelController : Kit.Common.Singleton<LevelController>
{
    ...
    [SerializeField] List<GridController> m_StageList;
    ...
    [ReadOnly] public GridController currentStage;
    [ReadOnly] public bool isUsingBooster;
    [ReadOnly] public bool isCollecting;
    [SerializedDictionary] public SerializedDictionary<int, List<StageData.CellType>> cellTypePerStageDict;
    public Camera mainCamera => m_MainCamera;
    public PictureController pictureController => m_PictureController;
    public List<GridController> stageList => m_StageList;
    ...
    public void SetupLevel()
    {
        ...
        currentStage = m_StageList[currentStageIndex];
        m_PictureController.Setup(levelData);

        //setup grid
        m_StageList.ForEach(x => x.Setup(levelData, m_StageList.IndexOf(x) + 1));
        ...
        cellTypePerStageDict = new SerializedDictionary<int, List<StageData.CellType>>();

        for (var i = 0; i < levelData.Stages.Length; i++)
        {
            var cellTypeList = new List<StageData.CellType>();
            foreach (var cell in levelData.Stages[i].Data)
            {
            ...
            cellTypePerStageDict[i] = cellTypeList;
        }
    }
    ...
    public void CheckStage()
    {
        ...
        var coilList = currentStage.GetCoilListInStage();
    ...
    {
        currentStageIndex += 1;
        if (currentStageIndex < levelData.Stages.Length)
        {
            currentStage = m_StageList[currentStageIndex];

            var offset = currentStage.transform.localPosition.x;
            m_StageList.ForEach(x => x.transform.DOLocalMoveX(x.transform.localPosition.x - offset, 0.5f));

            m_PictureController.NextStage();
            showOnNoCoinOnBoard = false;
        }
        else
        {
            if (!endGame)
            {
                ...
                m_PictureController.currentStage += 1;

                UILevelLabel.OnUpdatePhaseStatus?.Invoke(m_PictureController.currentStage);
                ...
                //demo.sprite = sprite;
                //m_WinAnimation.Play();
                //CanvasTestController.instance.ShowWinPanel();
            }
        }
    }
    ...
    public bool CheckCollectBooster()
    {
        ...
        var coilElementsInStage = currentStage.GetCoilListInStage();
        var tubeElementInStage = currentStage.tubeElements;
        var coilListInSlotExtra = m_SlotController.GetListCoilActiveInSlotExtra();
    ...
    public async UniTask RollCollectBooster()
    {
        ...
        foreach (var coilElement in collectList)
        {
            ...
            coilElement.FillToRollCollect(1f, currentStage.rollCollect.transform).ContinueWith(() =>
            {
        ...
        await collectList[0].RollCollectToPicture(1f, currentStage.rollCollect.transform);
        isCollecting = false;
    ...
    public async void SwapCoil(CoilElement coil1, CoilElement coil2)
    {
        ...
        coil1.MoveToShuffle(0.5f, currentStage.shuffleParticle.transform).Forget();
        coil2.MoveToShuffle(0.5f, currentStage.shuffleParticle.transform).Forget();
    ...
    {
        currentStage.shuffleParticle.Play();
        await UniTask.Delay(TimeSpan.FromSeconds(0.5f));
        currentStage.shuffleParticle.Stop();
    }

    private async UniTask ScaleRoll()
    {
        await UniTask.Delay(470);
        currentStage.roll.gameObject.SetActive(true);
        currentStage.roll.SetAutoStart(true);
        currentStage.roll.SetSpeedAnim(1f);
        var effect = PoolingManager.instance.mergeParticlePool.Get();
        effect.transform.position = currentStage.roll.transform.position;
        ...
        effect1.transform.position = currentStage.roll.transform.position;
        UniTask.Delay(500).ContinueWith(() => PoolingManager.instance.mergeParticlePool.Release(effect)).Forget();
        currentStage.roll.gameObject.SetActive(false);
    }

    private async UniTask ScaleRollCollect(Color color)
    {
        currentStage.rollCollect.ChangeColor(color);
        await UniTask.Delay(450);
        currentStage.rollCollect.gameObject.SetActive(true);
        currentStage.rollCollect.SetSpeedAnim(1f);
        var effect = PoolingManager.instance.mergeParticlePool.Get();
        effect.transform.position = currentStage.rollCollect.transform.position;
        UniTask.Delay(500).ContinueWith(() => PoolingManager.instance.mergeParticlePool.Release(effect)).Forget();
        //currentStage.roll.RevertRotate(1f);
        await UniTask.Delay(2200);
        currentStage.rollCollect.gameObject.SetActive(false);
    }
