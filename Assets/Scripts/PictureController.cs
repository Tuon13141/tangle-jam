...
public class PictureController : Kit.Common.Singleton<PictureController>
{
    ...
    [SerializeField] List<PrinterElement> m_Printers;
    ...
    public void Setup(LevelAsset levelData)
    {
        ...
        countStage = levelData.Stages.Length;
        currentStage = 1;
        ActionEvent.OnSetLevelPhase?.Invoke(currentStage, levelData);
        UILevelLabel.OnSetupPhaseItem?.Invoke(currentStage);
        TrackingManager.OnSetLevelPhase?.Invoke(currentStage);
        ...
        m_Printers[currentStage - 1].ChangeColorPinter(m_ColorPinActive);

        if (countStage == 2)
        {
            m_Printers[0].transform.SetLocalPosition(x: -0.165f);
            m_Printers[1].transform.SetLocalPosition(x: 0.165f);
            m_Printers[2].gameObject.SetActive(false);
        }

        //
        //Debug.Log(levelData.Stages[currentStage - 1].RandomCoils);
        var randomDataList = levelData.Stages[currentStage - 1].RandomCoils.Split(",").Select(x => int.Parse(x)).ToList();
        colorsInStage = randomDataList.Select(x => levelData.PixelData.Colors[x]).ToList();

        var a1 = 32 - levelData.Slices.Take(currentStage - 1).Sum();
        var a2 = 32 - levelData.Slices.Take(currentStage).Sum();
    ...
    public void NextStage()
    {
        ...
        UILevelLabel.OnUpdatePhaseStatus?.Invoke(currentStage);
        ...
        colorsInStage = levelData.Stages[currentStage - 1].RandomCoils.Split(",").Select(x => levelData.PixelData.Colors[int.Parse(x)]).ToList();
        m_Printers[currentStage - 1].ChangeColorPinter(m_ColorPinActive);
        for (int i = 0; i < currentStage - 1; i++)
        {
            m_Printers[i].ChangeColorPinter(m_ColorPinDone);
        }
    ...
    {
        m_Printers.ForEach(x => x.ChangeColorPinter(m_ColorPinDone));
    }
    ...
    {
        if (currentStage >= levelData.Stages.Length) return 0;
        return levelData.Stages[currentStage].Data.Count(t => t.isCoil && t.Value == colorIndex);
    }
